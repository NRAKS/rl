    ...
    def SeekIdeal(self):
        ...
        for _ in range(4**(self.layer+1)):
            ...
            while True:
                ...
                self.EvaluateNextState(CurrentState, Agent.GetSerectAction())
                NextState = self.GetNextState()
                Reward = self.EvaluateReward(CurrentState, Agent.GetSerectAction())
                Agent.Update(Reward)
                CurrentState = NextState
                if CurrentState >= self.GetGoalState():
                    Agent.InitSumReward()
                    break
