...
class MontyHall(Environment):
    ...
    def evaluate_next_state(self, action):
        ...
        # ヤギ(不正解)のドアをオープンする
        goat = random.choice(list(set(self.num_doors) - set([self.correct_answer, action])))
        # ヤギのドア番号を返す
        return goat
    ...
    def evaluate_reward(self, action):
        ...
        if action == 1:
            # 選んだドアの上書き
            self.serect_door = random.choice(list(set(self.num_doors) - set([self.serect_door, self.goat])))
